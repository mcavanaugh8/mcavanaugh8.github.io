    ...
    createDraftObjects() {
      ...
      if (players.length > 0) {
        players.forEach((player, index) => {
          ...
          for (let i = 0; i < teams.length; i++) {
            if (index === 0) {
              if (teams[i].querySelector('#real-draft-selection').querySelector('div') !== null) {
                pickName = teams[i].querySelector('.pick-name').textContent.replace(/\n|\s{2,}/g, '').replace(/^.*?\s/, '');
              } else {
                pickName = '';
              }
            ...
            if (teams[i].querySelectorAll('.player-name')[index] !== undefined && teams[i].querySelectorAll('.player-name')[index].textContent.includes('/')) {
              const picksArr =
          ...
          this.calculatePoints(draftObj, playerDraftObj, player, index, index);
        });
    ...
    addAllRounds(playerPicks, playerName, onReload) {
      ...
      if (playerPicks) {
        ...
        draftOrder.forEach((pick, index) => {
          ...
          participants.forEach((participant, index) => {
            const playerPickArr = participant.picks;
            const playerPicksDiv = document.querySelectorAll('.player-pick');
            
            playerPickArr.forEach((pick, pickIndex) => {
              playerPicksDiv[pickIndex].innerHTML = ``;
              playerPicksDiv[pickIndex].innerHTML += `<p class="player-name single-line">${pick}</p>`;
            });
          })
      } else {
        if (onReload) {
            ...
            prospects.forEach((prospect, index) => {
              if (prospect.name == pick.player) {
                let popoutString = `
                  ...
                  `;
                listItem.querySelector('#real-draft-selection').innerHTML = popoutString;
              }
            });
